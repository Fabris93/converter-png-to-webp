...
function convertToWebP(filePath) {
  ...
  const newFilePath = filePath.replace(/\.png$|\.jpg$|\.jpeg$/, '');
  const fileStat = fs.statSync(filePath);
  if (!fs.existsSync(newPath)) { //Utilizzare existsSync() al posto di isDirectory()
    console.log('newPath:::', newPath);
    fs.mkdirSync(newPath, { recursive: true });
  }

  const outputFilePath = newPath + '\\' + path.basename(filePath, path.extname(filePath)) + '.webp';
